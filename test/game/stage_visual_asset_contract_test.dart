...
void main() {
  ...
  test('tile bases are opaque and decals have transparent padded cells', () {
    for (final stageId in _stageIds) {
      ...
      for (var index = 0; index < 4; index++) {
        expect(png.cell(index).isFullyOpaque, isTrue, reason: '$stageId base $index');
      }
      for (var index = 4; index < 8; index++) {
        final cell = png.cell(index);
        expect(cell.hasTransparentBackground, isTrue, reason: '$stageId decal $index');
        expect(cell.hasVisibleContent, isTrue, reason: '$stageId decal $index');
        expect(cell.hasAtLeastPadding(4), isTrue, reason: '$stageId decal $index');
      }
    }
  });
  ...
  test('all eight prop cells are visible, transparent, and padded', () {
    for (final stageId in _stageIds) {
      ...
      for (var index = 0; index < 8; index++) {
        final cell = png.cell(index);
        expect(cell.hasTransparentBackground, isTrue, reason: '$stageId prop $index');
        expect(cell.hasVisibleContent, isTrue, reason: '$stageId prop $index');
        expect(cell.hasAtLeastPadding(4), isTrue, reason: '$stageId prop $index');
      }
    }
  });
  ...
  test('catalog and pubspec cover the exact runtime asset paths', () {
    expect(AssetCatalog.stages, {
      moonlitAbandonedOffice: 'assets/images/tiles/moonlit_office_tiles_128.png',
...
abstract final class _Sha256 {
  ...
  static String hash(List<int> input) {
    ...
    for (var offset = 0; offset < bytes.length; offset += 64) {
      ...
      for (var i = 16; i < 64; i++) {
        final s0 = _rightRotate(words[i - 15], 7) ^
            _rightRotate(words[i - 15], 18) ^
            (words[i - 15] >> 3);
        final s1 = _rightRotate(words[i - 2], 17) ^
  ...
  static int _rightRotate(int value, int amount) =>
      ((value.toUnsigned(32) >> amount) | (value << (32 - amount))) & 0xffffffff;
}
...
class _PngRgba {
  ...
  factory _PngRgba.read(String path) {
    ...
    for (var offset = 8; offset < bytes.length;) {
      ...
      if (type == 'IDAT') idat.add(bytes.sublist(offset + 8, offset + 8 + length));
      offset += length + 12;
    }
...
class _PngCell {
  ...
  bool hasAtLeastPadding(int amount) {
    for (var y = 0; y < 128; y++) {
      for (var x = 0; x < 128; x++) {
        if (x >= amount && x < 128 - amount && y >= amount && y < 128 - amount) continue;
        if (png.pixels[((top + y) * png.width + left + x) * 4 + 3] != 0) return false;
      }
    }
    return true;
  }
