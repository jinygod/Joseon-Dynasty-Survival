...
기본 디버그 웹 명령은 앱 전체를 가운데 정렬된 390x844 세로 프레임에서 실행한다.

```powershell
flutter run -d chrome
```

일반 웹 레이아웃과 비교할 때는 브라우저 전체 영역 모드를 사용한다.

```powershell
flutter run -d chrome --dart-define=MOBILE_PREVIEW=false
```

프리뷰는 디버그 웹에서만 표시되어야 한다. Android와 릴리스 빌드에는 바깥 배경,
프리뷰 여백, 합성 안전 영역이 나타나면 안 된다.

세로형 확인 항목:

1. Chrome 창을 크고 작게 바꿔도 390x844 게임 영역이 가운데 정렬되고 잘림이나
   찌그러짐 없이 contain 방식으로 유지되는지 확인한다.
2. HUD와 터치 조작 영역이 위아래 안전 영역 안에 유지되는지 확인한다.
3. 플레이어, 일반, 엘리트, 보스의 발 위치와 계층별 크기 차이를 확인하고 스프라이트가
   흐려지지 않는지 확인한다.
4. 자동 공격 중에도 이동이 계속되며 bobbing, 기울기, 반동이 과하지 않은지 확인한다.
5. 환도베기 이펙트가 범위 안의 가장 가까운 살아 있는 적을 향하고 실제 피해도 같은
   부채꼴에 적용되는지 확인한다.
6. 시각 크기 확대 후 밀집 웨이브의 겹침을 관찰한다. 카메라 줌이나 충돌 크기로 임시
   보정하지 말고 과도한 겹침을 기록한다.

## 전투 마스터리 수직 슬라이스 기록

전투 마스터리 빌드는 기존 원본 `playtest-run-log.csv`와
`run-telemetry.json`을 그대로 전달한다. 집계 수치는 원본을 대신하지 않는다.
동일한 로컬 빌드를 실제 Android 기기와 위의 390x844 Chrome 모바일 프리뷰에서
각각 실행한다. 이 절차는 로컬 검증용이며 배포하거나 외부 호스팅에 업로드하지 않는다.

자동 집계 항목과 실제 플레이 질문은
`docs/testing/combat-mastery-vertical-slice-report.md`에 기록한다. 자동화로 계산한
증거와 사람이 직접 관찰해야 하는 결과를 서로 섞지 말고, 실제로 확인하기 전에는
관찰 결과 체크박스를 비워 둔다.
